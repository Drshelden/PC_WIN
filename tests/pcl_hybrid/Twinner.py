...
class PCLProcessorSettingsForm(EtoForms.Dialog[bool]):
    ...
    def __init__(self):
        ...
        # Initialize logging for UI operations (if not already initialized by rhino_client import)
        try:
            import pcl_hybrid_py as ph
            if hasattr(ph, 'log_message'):
                ph.log_message("Twinner: PCL Processor Settings UI initialized")
        except:
            pass
    ...
    def SaveSettingsToFile(self, file_path):
        ...
        try:
            import pcl_hybrid_py as ph
            if hasattr(ph, 'log_message'):
                ph.log_message(f"Rhino3DPO: Settings saved to {file_path}")
        except:
            pass
    ...
    def LoadSettingsFromFile(self, file_path):
        ...
        try:
            ...
            # Update the UI controls to reflect the loaded settings
            self.UpdateUIFromParameters()
            
            # Log the settings load operation
            try:
                import pcl_hybrid_py as ph
                if hasattr(ph, 'log_message'):
                    ph.log_message(f"Twinner: Settings loaded from {file_path}")
            except:
                pass
            ...
            try:
                import pcl_hybrid_py as ph
                if hasattr(ph, 'log_message'):
                    ph.log_message(f"Twinner: Error loading settings from {file_path}: {ex}")
            except:
                pass
            return False
